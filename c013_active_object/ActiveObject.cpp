...
bool ActiveObject::_main() {
	...
	while( _state != State::STOPPED ) {
		...
		case ActiveObject::State::CREATED:
			LOG4CPLUS_INFO( _logger, *this << " enter the main_loop!" );
			break;

		case ActiveObject::State::INITIALIZING :
			if ( doInitilize() ) {
				LOG4CPLUS_INFO( _logger, *this << " doInitilize() succeed" );
				_state = State::READY;
			} else {
		...
		case ActiveObject::State::READY:
			LOG4CPLUS_INFO( _logger, *this << " waiting for start..." );
			break;
		...
		case ActiveObject::State::FINALIZING   :
			if ( doFinalize() ) {
				LOG4CPLUS_INFO( _logger, *this << " doFinalize() succeed" );
			} else {
